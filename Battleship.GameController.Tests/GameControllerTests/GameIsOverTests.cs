namespace Battleship.GameController.Tests.GameControllerTests
{
    ...
    public class GameIsOverTests
    {
        ...
        public void SunkShipsListed()
        {
            ...
            for (int i = 0; i < 3; i++) //only sink the first three
            {
                foreach (var pos in shipPositions[i])
                {
                    var isHit = GameController.CheckIsHit(ships, pos);
        ...
        public void NoSunkShipsListed()
        {
            ...
            var sunkShips = GameController.ListSunkShips(ships);

            Assert.IsTrue(sunkShips.Count() == 0);
        }
    }
}